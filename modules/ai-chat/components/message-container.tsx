...
const MessageTypeIndicator: React.FC<{
  ...
  const getTypeConfig = (type?: string) => {
    switch (type) {
      case "code_review":
        return { icon: Code, color: "text-blue-600", label: "Code Review" };
      case "suggestion":
        return {
          icon: Sparkles,
      ...
      case "error_fix":
        return { icon: RefreshCw, color: "text-red-600", label: "Error Fix" };
      case "optimization":
        return { icon: Zap, color: "text-yellow-600", label: "Optimization" };
      default:
        return { icon: MessageSquare, color: "text-orange-600", label: "Chat" };
    }
  };
  ...
  return (
    <div className="flex items-center justify-between mb-2">
      <div className="flex items-center gap-1">
        <Icon className={cn("h-3 w-3 font-bold", config.color)} />
...
const MessageContainer: React.FC<MessageContainerProps> = ({
  ...
  return (
    <div className="flex-1 overflow-y-auto dark:bg-black bg-white rounded-lg p-4">
              ...
              <div
                className={cn(
                  ...
                  msg.role === "user"
                    ...
                    : "bg-neutral-300 dark:bg-muted-foreground backdrop-blur-sm text-black   p-5 rounded-bl-md border "
                )}
              >
                {msg.role === "model" && (
